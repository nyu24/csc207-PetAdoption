...
public class SetParamView extends JPanel implements PropertyChangeListener {
    ...
    public SetParamView(SetParamViewModel setParamViewModel) {
        ...
        // type drop down ----------------
        final JPanel typeDropdownPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        typeDropdownPanel.add(new JLabel("Animal Type: "));
        typeDropdownPanel.add(typeDropdown);

        // panels for the other attributes
        final JPanel breedDropdownPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        breedDropdownPanel.add(new JLabel("Breed Type: "));
        breedDropdownPanel.add(breedDropdown);

        final JPanel coatDropdownPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        coatDropdownPanel.add(new JLabel("Coat Length: "));
        coatDropdownPanel.add(coatDropdown);

        final JPanel colorDropdownPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        colorDropdownPanel.add(new JLabel("Color: "));
        colorDropdownPanel.add(colorDropdown);

        final JPanel genderDropdownPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        ...
        startBtn.addActionListener(
                e -> {
                    ...
                    typeDropdown.addActionListener(
                            e1 -> {
                                final JComboBox comboBox = (JComboBox) e1.getSource();
                                final String type = (String) comboBox.getSelectedItem();

                                // type attributes if type is NOT empty-----------------
                                if (!type.isEmpty()) {
                                    final ArrayList<ArrayList<String>> attributesList = setParamController.getTypeAttributes(type);
